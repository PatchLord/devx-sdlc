> **ARCHIVED, and kept on purpose.** This is SOW v1.0 as a single 32,000-word file, before it was split
> into a signature document plus annexes. It is preserved because it is evidence of a real failure mode:
> every addition in it was demanded by a reviewer and every one is defensible on its own, and the aggregate
> is a document a founder will not read. Disclosure has a length budget, and past some point disclosure
> becomes concealment — the sentence you most need someone to see ends up on page forty.
>
> The split version is at `docs/sow.md` with `docs/annexes/`. Nothing was deleted in the split; the annexes
> hold the detail verbatim.
